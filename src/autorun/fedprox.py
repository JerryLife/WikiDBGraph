"""
FedProx Training Script for Automated FL Validation

This script runs FedProx federated learning on automatically processed data pairs.
FedProx adds a proximal term to limit local model drift from the global model.
"""
...
from model.FedProx import FedProx
...
def load_auto_processed_data(data_dir: str, config: dict) -> tuple:
    ...
    # Prepare data dictionaries for FedProx (uses DataFrame format)
    train_data = {
        'client_0': train_a,
        'client_1': train_b
    }
...
def train_fedprox_model(train_data: dict, test_data: dict, task_info: dict,
    ...
    """
    Train FedProx model for classification or regression task.
    ...
    Args:
        ...
        mu: Proximal term parameter (controls local drift)
    ...
    print(f"Training FedProx with parameters:")
    print(f"  Task type: {task_type}")
    print(f"  Output dimension: {output_dim}")
    ...
    fedprox.fit(train_data, target_col=label_col, test_data_dict=test_data)
    ...
    test_results = fedprox.eval(combined_test_dict, target_col=label_col)
    ...
    print(f"FedProx Results (Combined Test Set):")
...
def main():
    """Main function for FedProx training."""
    parser = argparse.ArgumentParser(description="FedProx training for automated FL validation")
    ...
    parser.add_argument("--local-epochs", type=int, default=5,
                       help="Local epochs for FedProx (default: 5)")
    parser.add_argument("--global-rounds", type=int, default=20,
                       help="Global rounds for FedProx (default: 20)")
    parser.add_argument("--mu", type=float, default=0.001,
                       help="Proximal term parameter (default: 0.001)")
    ...
    log_file = output_dir / 'fedprox_errors.log'
    ...
    print("\n" + "="*60)
    print(f"Running FedProx Training ({task_info['task_type']})")
    print("="*60)
    
    try:
        fedprox_results = train_fedprox_model(
        ...
        logging.error(f"An error occurred during FedProx training: {e}")
        logging.error(f"Full traceback: {traceback.format_exc()}")
        print(f"Error: FedProx training failed: {e}")
        print(f"Full error details logged to: {log_file}")
        return 1
    ...
    # Print summary
    print("\n" + "="*60)
    print("FEDPROX EXPERIMENT SUMMARY")
    print("="*60)
    print(f"Pair: {config['pair_id']}")
    print(f"Task type: {task_info['task_type']}")
    print(f"Similarity: {config['similarity']:.4f}")
    print(f"Features: {num_features}")
    print(f"FedProx Accuracy: {fedprox_results['accuracy']:.4f}")
    print(f"FedProx F1 Score: {fedprox_results['f1']:.4f}")
