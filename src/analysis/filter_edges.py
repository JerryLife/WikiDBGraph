...
def determine_threshold(df):
    ...
    threshold = ground_truth_df['similarity'].quantile(0.25)  # 25th percentile means 75% of values are above this
...
def main(input_file="data/graph/all_exhaustive_predictions.pt", output_path=None):
    ...
    if output_path is None:
        output_path = f"data/graph/filtered_edges_threshold_{threshold:.2f}.csv"
    else:
        # Insert threshold into filename
        base, ext = os.path.splitext(output_path)
        output_path = f"{base}_threshold_{threshold:.6f}{ext}"
...
if __name__ == "__main__":
    main(
        input_file="data/graph/all_exhaustive_predictions.pt",
