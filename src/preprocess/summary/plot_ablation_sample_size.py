...
Shows AUC-ROC (and optionally F1-Score) vs sample size with error bars.
"""
...
def plot_ablation_sample_size(
    ...
    print("=" * 70)
    print(f"{'Size':<10} {'AUC':<10} {'AUC_std':<10} {'F1':<10} {'F1_std':<10}")
    print("-" * 70)
    
    for size in sizes:
        metrics = collect_size_results(results_dir, size)
        if 'auc' in metrics:
            ...
            print(f"{size:<10} {metrics['auc']:<10.4f} {metrics.get('auc_std', 0.0):<10.4f} "
                  f"{metrics.get('f1', 'N/A'):<10} {metrics.get('f1_std', 0.0):<10.4f}")
        else:
            print(f"{size:<10} {'N/A':<10} {'N/A':<10} {'N/A':<10} {'N/A':<10}")
    ...
    ax.set_title('Effect of Sample Size', fontsize=FONTSIZE['title'], fontweight='bold', pad=15)
