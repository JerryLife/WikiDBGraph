...
def build_dgl_graph_from_pt(pt_file_path, device=None, num_nodes=100000):
    ...
    g.edata["weight"] = valid_edges[:, 2].to(device) if device else valid_edges[:, 2]
...
def build_dgl_graph_from_csv(csv_file_path, device=None, num_nodes=100000):
    ...
    # Create graph with explicit number of nodes to ensure all nodes are included
    g = dgl.graph((src, dst), num_nodes=num_nodes, device=device)
    g.edata["weight"] = torch.tensor(df['similarity'].values, dtype=torch.float32).to(device) if device else torch.tensor(df['similarity'].values, dtype=torch.float32)
    
    # Add ground truth label if available
    if 'label' in df.columns:
        g.edata["gt_edge"] = torch.tensor(df['label'].values, dtype=torch.float32).to(device) if device else torch.tensor(df['label'].values, dtype=torch.float32)
...
if __name__ == "__main__":
    # Example usage
    edges_file = "data/graph/filtered_edges_threshold_0.96.csv"
    embeddings_file = "data/graph/database_embeddings.pt"
    output_path = "data/graph/graph_raw.dgl"
