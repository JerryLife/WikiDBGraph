...
def save_results(results, algorithm_name="fedavg", seed=0, database_ids=None):
...
def train_fedavg_model(train_data, test_data,
                      num_classes=2,
                      hidden_dims=[64, 32],
                      learning_rate=0.001,
                      local_epochs=5,
    ...
    """
    Train and evaluate FedAvg model.
    """
    print(f"\n=== Training FedAvg Model ===")
    ...
    # Pre-set the unified label encoder to prevent FedAvg from creating its own
    if unified_encoder is not None:
        model.label_encoder = unified_encoder
        print(f"Applied unified label encoder to FedAvg model")
...
def compare_with_centralized(train_data, test_data, num_classes=2, target_col=None, unified_encoder=None, unified_scaler=None):
    """
    Compare FedAvg with centralized learning baseline using simple neural network.
    """
...
if __name__ == "__main__":
    ...
    save_results(results, "fedavg", seed=args.seed, database_ids=args.databases)