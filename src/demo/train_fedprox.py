...
def save_results(results, algorithm_name="fedprox", seed=0, database_ids=None):
...
def train_fedprox_model(train_data, test_data,
                       num_classes=2,
                       hidden_dims=[64, 32],
                       learning_rate=0.001,
                       local_epochs=5,
    ...
    """
    Train and evaluate FedProx model.
    """
    print(f"\n=== Training FedProx Model ===")
    ...
    model = FedProx(
        ...
        mu=mu  # FedProx proximal term parameter
    )
    
    # Pre-set the unified label encoder to prevent FedProx from creating its own
    if unified_encoder is not None:
        model.label_encoder = unified_encoder
        print(f"Applied unified label encoder to FedProx model")
...
def compare_with_centralized(train_data, test_data, num_classes=2, target_col=None, unified_encoder=None, unified_scaler=None):
    """
    Compare FedProx with centralized learning baseline using simple neural network.
    """
...
if __name__ == "__main__":
    ...
    MU_VALUE = 0.01  # FedProx proximal term parameter
    ...
    print(f"\n{'='*60}")
    print(f"Running experiment: {experiment['name']} (mu={experiment['mu']})")
    print(f"{'='*60}")
    
    result = train_fedprox_model(
    ...
    save_results(results, "fedprox", seed=args.seed, database_ids=args.databases)