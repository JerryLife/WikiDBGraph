"""
FedOV Training Script for Automated FL Validation

This script runs FedOV (one-shot federated learning with voting) on automatically processed data pairs.
FedOV trains clients independently in a single round and uses voting for predictions.
"""
...
def load_auto_processed_data(data_dir: str, config: dict) -> tuple:
    ...
    # Prepare data dictionaries for FedOV (uses DataFrame format)
    train_data = {
        'client_0': train_a,
        'client_1': train_b
    }
...
def train_fedov_model(train_data: dict, test_data: dict, task_info: dict,
    ...
    """
    Train FedOV model for classification or regression task.
    ...
    Args:
        ...
        local_epochs: Number of local epochs (higher for one-shot)
    ...
    print(f"Training FedOV with parameters:")
    print(f"  Task type: {task_type}")
    print(f"  Output dimension: {output_dim}")
    print(f"  Hidden dims: {hidden_dims}")
    print(f"  Learning rate: {learning_rate}")
    print(f"  Local epochs: {local_epochs}")
    print(f"  Global rounds: 1 (one-shot)")
    ...
    fedov = FedOV(
        ...
        global_rounds=1,  # Always 1 for FedOV (one-shot)
    ...
    # Train the model
    print("Starting FedOV training (one-shot)...")
    fedov.fit(train_data, target_col=label_col, test_data_dict=test_data)
    ...
    test_results = fedov.eval(combined_test_dict, target_col=label_col)
    ...
    print(f"FedOV Results (Combined Test Set):")
...
def main():
    """Main function for FedOV training."""
    parser = argparse.ArgumentParser(description="FedOV training for automated FL validation")
    ...
    parser.add_argument("--local-epochs", type=int, default=10,
                       help="Local epochs for FedOV (default: 10, higher for one-shot)")
    parser.add_argument("--augmentation-rate", type=float, default=0.3,
                       help="Data augmentation rate for outlier generation (default: 0.3)")
    parser.add_argument("--outlier-threshold", type=float, default=0.5,
                       help="Outlier detection threshold (default: 0.5)")
    ...
    log_file = output_dir / 'fedov_errors.log'
    ...
    # Run FedOV training
    print("\n" + "="*60)
    print(f"Running FedOV Training ({task_info['task_type']})")
    print("="*60)
    
    try:
        fedov_results = train_fedov_model(
        ...
        logging.error(f"An error occurred during FedOV training: {e}")
        logging.error(f"Full traceback: {traceback.format_exc()}")
        print(f"Error: FedOV training failed: {e}")
        print(f"Full error details logged to: {log_file}")
        return 1
    ...
    # Print summary
    print("\n" + "="*60)
    print("FEDOV EXPERIMENT SUMMARY")
    print("="*60)
    print(f"Pair: {config['pair_id']}")
    print(f"Task type: {task_info['task_type']}")
    print(f"Similarity: {config['similarity']:.4f}")
    print(f"Features: {num_features}")
    print(f"FedOV Accuracy: {fedov_results['accuracy']:.4f}")
    print(f"FedOV F1 Score: {fedov_results['f1']:.4f}")
