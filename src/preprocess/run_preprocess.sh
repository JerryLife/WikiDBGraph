...
if [[ $SKIP_TRIPLETS -eq 0 ]]; then
    ...
    if [[ -f "$QID_PAIRS" ]] && [[ -f "$NEGATIVE_POOL" ]]; then
    ...
    log "Skipping triplet generation"
fi
...
if [[ $SKIP_TRAINING -eq 0 ]]; then
    ...
    if [[ -z "$MODEL_PATH" ]]; then
        # Check if finetuned model exists in current output directory
        MODEL_SAVE_DIR="${OUTPUT_DIR}/weights/finetuned_bge_m3_softmax_lr${LR}"
        ...
        if [[ -d "$CANDIDATE_PATH" ]]; then
            ...
            if [[ -d "$LEGACY_PATH" ]]; then
                ...
                # Fallback to pretrained model (empty string means use BAAI/bge-m3)
                MODEL_PATH=""
...
if [[ $SKIP_EMBEDDINGS -eq 0 ]]; then
    ...
    log "Skipping embedding generation"
fi
...
if [[ $SKIP_SIMILARITY -eq 0 ]]; then
    ...
    log "Skipping similarity computation"
fi
