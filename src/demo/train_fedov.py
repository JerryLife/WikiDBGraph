...
def save_results(results, algorithm_name="fedov", seed=0, database_ids=None):
...
def train_fedov_model(train_data, test_data,
                     num_classes=2,
                     hidden_dims=[64, 32],
                     learning_rate=0.001,
                     local_epochs=15,  # More epochs for one-shot learning
                     batch_size=32,
                     device='cpu',
    ...
    """
    Train and evaluate FedOV model.
    """
    print(f"\n=== Training FedOV Model ===")
    ...
    model = FedOV(
        ...
        global_rounds=1,  # Always 1 for FedOV
    ...
    # Pre-set the unified label encoder to prevent FedOV from creating its own
    if unified_encoder is not None:
        model.label_encoder = unified_encoder
        print(f"Applied unified label encoder to FedOV model")
    ...
    # Evaluate on test data with different voting strategies
    print("\\n--- Evaluation Phase ---")
    try:
        # Test with different numbers of accepted votes
        num_clients = len(train_data)
        voting_results = {}
        
        for accepted_votes in range(1, num_clients + 1):
            print(f"\\n--- Evaluating with {accepted_votes} accepted votes ---")
            client_results = model.eval(test_data, target_col=target_col, accepted_votes=accepted_votes)
            ...
            print(f"\\nWeighted Average Results (votes={accepted_votes}):")
            ...
            voting_results[accepted_votes] = weighted_metrics
        
        # Find best voting strategy
        best_votes = max(voting_results.keys(), key=lambda x: voting_results[x]['f1'])
        best_metrics = voting_results[best_votes]
        
        print(f"\\n=== Best Results ===")
        print(f"Best voting strategy: {best_votes} accepted votes")
...
def compare_with_centralized(train_data, test_data, num_classes=2, target_col=None, unified_encoder=None, unified_scaler=None):
    """
    Compare FedOV with centralized learning baseline using simple neural network.
    """
...
if __name__ == "__main__":
    # Parse arguments
    parser = argparse.ArgumentParser(description="FedOV federated learning experiment")
    ...
    # FedOV experiment configuration
    experiment = {
        'name': 'FedOV',
        'hidden_dims': [64, 32],
        'local_epochs': 15,  # More epochs for one-shot learning
    ...
    # Run FedOV experiment
    print(f"\\n{'='*60}")
    print(f"Running experiment: {experiment['name']}")
    print(f"{'='*60}")
    
    result = train_fedov_model(
        ...
        device=DEVICE,
        target_col=TARGET_COL,
        augmentation_rate=experiment['augmentation_rate'],
        outlier_threshold=experiment['outlier_threshold'],
        unified_encoder=unified_encoder
    )
    
    if result:
        results[experiment['name']] = result['best_metrics']
        
        # Print voting strategy results
        print(f"\\n=== Voting Strategy Analysis ===")
        for votes, metrics in result['voting_results'].items():
            print(f"Votes {votes}: Acc={metrics['accuracy']:.4f}, F1={metrics['f1']:.4f}")
    ...
    if results:
        ...
        # Special note about FedOV
        if best_method == 'FEDOV':
            print(f"\\n📊 FedOV Analysis:")
            print(f"   - One-shot communication (single round)")
            print(f"   - Best voting strategy: {result['best_votes']} clients")
            print(f"   - Outlier detection enabled with extra class")
            print(f"   - Data augmentation rate: {experiment['augmentation_rate']}")
    
    # Save results to JSON file
    save_results(results, "fedov", seed=args.seed, database_ids=args.databases)